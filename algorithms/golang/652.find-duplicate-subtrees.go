import (
	"fmt"
)

/*
 * @lc app=leetcode id=652 lang=golang
 *
 * [652] Find Duplicate Subtrees
 *
 * https://leetcode.com/problems/find-duplicate-subtrees/description/
 *
 * algorithms
 * Medium (49.79%)
 * Likes:    1263
 * Dislikes: 198
 * Total Accepted:    61.7K
 * Total Submissions: 123.8K
 * Testcase Example:  '[1,2,3,4,null,2,4,null,null,4]'
 *
 * Given a binary tree, return all duplicate subtrees. For each kind of
 * duplicate subtrees, you only need to return the root node of any one of
 * them.
 *
 * Two trees are duplicate if they have the same structure with same node
 * values.
 *
 * Example 1:
 *
 *
 * ⁠       1
 * ⁠      / \
 * ⁠     2   3
 * ⁠    /   / \
 * ⁠   4   2   4
 * ⁠      /
 * ⁠     4
 *
 *
 * The following are two duplicate subtrees:
 *
 *
 * ⁠     2
 * ⁠    /
 * ⁠   4
 *
 *
 * and
 *
 *
 * ⁠   4
 *
 * Therefore, you need to return above trees' root in the form of a list.
 */

// @lc code=start
/**
 * Definition for a binary tree node.
 * type TreeNode struct {
 *     Val int
 *     Left *TreeNode
 *     Right *TreeNode
 * }
 */
func findDuplicateSubtrees(root *TreeNode) []*TreeNode {
	hash := make(map[string]int, 0)
	retVal := make([]*TreeNode, 0)
	dfs(root, hash, &retVal)
	return retVal
}

func dfs(root *TreeNode, treeHash map[string]int, retVal *[]*TreeNode) string {
	if root == nil {
		return "#"
	}

	leftTree := dfs(root.Left, treeHash, retVal)
	rightTree := dfs(root.Right, treeHash, retVal)

	serial := fmt.Sprintf("%d,%s,%s", root.Val, leftTree, rightTree)

	treeHash[serial]++
	if val, ok := treeHash[serial]; ok {
		if val == 2 {
			*retVal = append(*retVal, root)
		}
	}
	// fmt.Println(serial)
	// fmt.Println(treeHash)
	return serial
}

// @lc code=end