...
func openLock(deadends []string, target string) int {
	...
	directions := []int{-1, 1} // two dirctions, add one or minus one
	queue, rotate := []string{"0000"}, 0
	visited["0000"] = true
	for len(queue) > 0 {
		newQueue := []string{}
		for _, item := range queue {
			...
			for i := 0; i < len(item); i++ {
				for _, direction := range directions { // iterat two directions
					newVal := (int(item[i]-'0') + direction + 10) % 10
					newItem := item[:i] + strconv.Itoa(newVal) + item[i+1:] // change direction to make new item
